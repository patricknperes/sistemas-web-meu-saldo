    ...
    return (
        <div className={`w-full select-none ${className}`}>
            <div className="flex items-center justify-between gap-3">
                ...
                <p
                    className="min-w-0 text-center text-body-sm font-bold capitalize text-foreground"
                    aria-live="polite"
                >
            ...
            <div className="mt-4 grid grid-cols-7 gap-1" aria-hidden="true">
                {WEEKDAYS_SHORT.map((weekday, index) => (
                    <span
                        key={`${weekday}-${index}`}
                        className="flex h-7 items-center justify-center text-overline font-bold uppercase text-subtle-foreground"
                    >
                        {weekday}
                    </span>
                ))}
